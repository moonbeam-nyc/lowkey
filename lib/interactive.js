...
async function fuzzyPrompt(question, choices, displayFn = null, breadcrumbs = [], errorMessage = null, initialQuery = '') {
  ...
  return new Promise((resolve) => {
    ...
    let searchMode = initialQuery.length > 0; // Start in search mode if we have an initial query
    ...
    function doRender() {
      try {
        ...
        if (searchMode || query.length > 0) {
          const displayQuery = searchMode ? query + '█' : query; // Show cursor block when in search mode
        ...
        if (filteredChoices.length === 0) {
          ...
          // Show "previous items" indicator at top if there are hidden items above
          if (startIndex > 0) {
          ...
          // Show "more items" indicator at bottom if there are hidden items below
    ...
    process.stdin.on('data', (key) => {
      ...
      if (keyStr === '\u0003') { // Ctrl+C
        ...
        if (searchMode) {
          ...
          rl.close();
          resolve({
            selection: filteredChoices[selectedIndex],
            query: query
          });
        }
        ...
        render(true); // Immediate for navigation
      } else if (keyStr === '\u0015') { // Ctrl+U - Page up
        const terminalHeight = process.stdout.rows || 24;
        const pageSize = Math.max(1, Math.floor(terminalHeight / 2));
        selectedIndex = Math.max(0, selectedIndex - pageSize);
        render(true);
      } else if (keyStr === '\u0004') { // Ctrl+D - Page down
        const terminalHeight = process.stdout.rows || 24;
        const pageSize = Math.max(1, Math.floor(terminalHeight / 2));
        selectedIndex = Math.min(filteredChoices.length - 1, selectedIndex + pageSize);
        render(true);
      } else if (keyStr === '\u0002') { // Ctrl+B - Page up (alternative)
        const terminalHeight = process.stdout.rows || 24;
        const pageSize = Math.max(1, Math.floor(terminalHeight / 2));
        selectedIndex = Math.max(0, selectedIndex - pageSize);
        render(true);
      } else if (keyStr === '\u0006') { // Ctrl+F - Page down (alternative)
        const terminalHeight = process.stdout.rows || 24;
        const pageSize = Math.max(1, Math.floor(terminalHeight / 2));
        selectedIndex = Math.min(filteredChoices.length - 1, selectedIndex + pageSize);
        render(true);
        ...
        // Note: Removed automatic search mode activation - user must press '/' first
      }
      } catch (error) {
