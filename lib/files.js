...
const { FILES, ENV } = require('./constants');
  ...
  try {
    ...
    for (const line of lines) {
      ...
      if (trimmed && !trimmed.startsWith('#')) {
        const match = trimmed.match(ENV.LINE_PATTERN);
        if (match) {
          let [, key, value] = match;
...
function listJsonFiles(directory) {
  try {
    ...
    const excludeFiles = FILES.JSON_EXCLUDE_LIST;
...
function validateEnvKey(key) {
  return ENV.KEY_PATTERN.test(key);
}
...
function backupFile(filePath) {
  if (fs.existsSync(filePath)) {
    const backupPath = `${filePath}${FILES.BACKUP_EXTENSION}`;
