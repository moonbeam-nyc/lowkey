...
class CommandParser {
   ...
   * @param {Object} config.customArgs - Custom argument definitions
   * @param {Array} config.requiredArgs - Required argument field names
   * @param {Function} config.showHelp - Help display function
   * @param {string} config.commandName - Command name for defaults
   * @param {boolean} config.requiresStorageType - Whether command needs --type validation
   * @param {boolean} config.requiresAwsRegion - Whether to validate AWS region when using AWS
   * @param {boolean} config.requiresKubernetesNamespace - Whether to require --namespace for kubernetes
   * @param {Array} config.supportedTypes - Override default supported types
   * @returns {Object} Parsed and validated options
   */
  static parseCommand(args, config) {
    const {
      customArgs = {},
      requiredArgs = [],
      showHelp,
