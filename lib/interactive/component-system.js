...
const Menu = (items = [], selectedIndex = 0, options = {}) =>
  new Component('menu', {
    ...
    keyColor: options.keyColor || 'yellow',
...
const KeyBinding = (key = '', description = '', keyColor = 'yellow') =>
...
const InstructionsFromOptions = (options = {}) => {
  ...
  if (options.hasPageNavigation) {
    bindings.push({ key: 'Ctrl+U/D', description: 'page up/down' });
  }
  ...
  if (options.hasEdit) {
    bindings.push({ key: 'e', description: 'edit' });
  }
  
  if (options.hasCopy) {
    bindings.push({ key: 'Ctrl+S', description: 'copy' });
  }
  
  if (options.hasToggle) {
    bindings.push({ key: 'Ctrl+V', description: 'toggle values' });
  }
  ...
  bindings.push({ key: 'Ctrl+A', description: 'AWS profile' });
