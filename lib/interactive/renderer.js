...
class ScreenRenderer {
  ...
  doRender(state) {
    ...
    try {
      // Clear screen using centralized utility
      terminal.clearScreen();
      
      // Get the global header from TerminalManager
      const { TerminalManager } = require('./terminal-manager');
      const terminalManager = TerminalManager.getInstance();
      const headerLines = terminalManager.getHeaderLines();
      
      // Render header if available
      if (headerLines && headerLines.length > 0) {
        terminal.write(headerLines.join('\n') + '\n');
      }
      ...
      output.error(`Render error: ${error.message}`);
    }
  }
...
const RenderUtils = {
  ...
  calculateAvailableHeight(usedLines) {
    ...
    // Account for global header (3 lines: header, separator, blank)
    const { TerminalManager } = require('./terminal-manager');
    const terminalManager = TerminalManager.getInstance();
    const headerLines = terminalManager.headerEnabled ? 3 : 0;
    
    const totalUsedLines = usedLines + headerLines + INTERACTIVE.RESERVED_LINES_FOR_UI;
  ...
  getPaginationIndicators(startIndex, endIndex, totalItems) {
    ...
    if (startIndex > 0) {
      indicators.push(colorize(`⋮ ${startIndex} more above`, 'gray'));
    }
    
    if (endIndex < totalItems) {
      const remaining = totalItems - endIndex;
      indicators.push(colorize(`⋮ ${remaining} more below`, 'gray'));
    }
  ...
  formatInstructions(hasBackNavigation = false, hasEdit = false, hasCopy = false) {
    ...
    const copy = hasCopy ? 'Ctrl+S to copy, ' : '';
    ...
    return colorize(`${navigation}, ${search}, ${action}, ${edit}${copy}${values}${back}${exit}`, 'gray');
  },
