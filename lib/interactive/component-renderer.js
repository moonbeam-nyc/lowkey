...
class ComponentRenderer {
  ...
  renderComponent(component) {
    ...
    switch (component.type) {
      ...
      case 'spinner':
        return this.renderSpinner(component);
      case 'table':
  ...
  renderSearchInput(component) {
    ...
    const prefix = isActive ? colorize('> ', 'green') : '';
    
    // Show placeholder with cursor when actively searching but no query yet
    if (isActive && !query) {
      return `${prefix}${colorize(placeholder, 'gray')} ${cursor}`;
    }
    
    return `${prefix}Search: ${query}${cursor}`;
  }
  ...
  renderList(component) {
    ...
    for (let i = startIndex; i < endIndex; i++) {
      ...
      // Add selection indicator (only if showSelectionIndicator is true)
      let prefix = '';
      if (showSelectionIndicator) {
        prefix = isSelected ? colorize(selectionIndicator, 'green') : ' '.repeat(selectionIndicator.length);
      }
      
      // Apply selection highlighting (only if showSelectionIndicator is true and item is selected)
  ...
  renderSpinner(component) {
    const { message = 'Loading...', frames = ['⠋', '⠙', '⠹', '⠸', '⠼', '⠴', '⠦', '⠧', '⠇', '⠏'], frameIndex = 0 } = component.props;
    
    // Use current frame from the animation
