...
class ComponentRenderer {
  ...
  renderTextInput(component) {
    ...
    let displayContent = value || colorize(placeholder, 'gray');
    
    // Add cursor if active
    if (showCursor) {
      const beforeCursor = displayContent.slice(0, cursorPosition);
      const afterCursor = displayContent.slice(cursorPosition);
  ...
  renderInstructions(component) {
    ...
    const parts = bindings.map(binding => {
      const key = colorize(binding.key, binding.keyColor || 'yellow');
