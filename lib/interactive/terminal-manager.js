...
class TerminalManager {
  ...
  getHeaderLines(breadcrumbs = []) {
    ...
    const awsInfo = this.headerInfo.awsProfile === 'unavailable' 
      ...
      : colorize(`aws: ${this.headerInfo.awsProfile}@${this.headerInfo.awsRegion}`, 'yellow');
    ...
    const k8sInfo = this.headerInfo.k8sContext === 'unavailable'
      ...
      : colorize(`k8s: ${this.headerInfo.k8sContext}`, 'green');
    ...
    if (breadcrumbs && breadcrumbs.length > 0) {
      const breadcrumbText = breadcrumbs.join(colorize(' > ', 'gray'));
      lines.push(colorize(breadcrumbText, 'gray'));
    }
