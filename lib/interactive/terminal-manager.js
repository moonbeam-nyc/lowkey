...
class TerminalManager {
  ...
  async loadHeaderInfo() {
    ...
    try {
      // Load AWS profile and region using proper config system
      const { getCurrentProfile, getCurrentRegion } = require('../utils/aws-config');
      this.headerInfo.awsProfile = getCurrentProfile();
      this.headerInfo.awsRegion = getCurrentRegion() || 'unset';
    } catch (error) {
  ...
  handleGlobalKeys(keyStr) {
    ...
    // Ctrl+K - Kubernetes Context Popup (global)
    if (keyStr === '\u000b') {
      debugLogger.log('TerminalManager.handleGlobalKeys', 'Ctrl+K detected - showing Kubernetes context popup');
      this.showGlobalKubernetesContextPopup();
      return true;
    }
    ...
    try {
      ...
      const popup = new AwsProfilePopup({
        onConfigChange: (config) => {
          ...
          this.headerInfo.awsRegion = config.region || 'unset';
  ...
  // Show Kubernetes context popup globally
  showGlobalKubernetesContextPopup() {
    const debugLogger = require('../core/debug-logger');
    
    try {
      debugLogger.log('TerminalManager.showGlobalKubernetesContextPopup', 'Showing global Kubernetes context popup');
      
      const { getPopupManager } = require('./popup-manager');
      const KubernetesContextPopup = require('./screens/kubernetes-context-screen');
      
      const popupManager = getPopupManager();
      
      const popup = new KubernetesContextPopup({
        onConfigChange: (config) => {
          debugLogger.log('TerminalManager.showGlobalKubernetesContextPopup', 'Kubernetes configuration changed globally', config);
          
          // Update global header information
          this.headerInfo.kubernetesContext = config.context || 'none';
          
          // Trigger header refresh
          this.refreshHeaderInfo();
          
          // Notify current screen about Kubernetes config change (for screen-specific refresh logic)
          if (this.currentScreen && typeof this.currentScreen.onKubernetesConfigChange === 'function') {
            this.currentScreen.onKubernetesConfigChange(config);
          }
      ...
      debugLogger.log('TerminalManager.showGlobalKubernetesContextPopup', 'Error showing global Kubernetes context popup', {
        error: error.message,
        stack: error.stack
      });
      ...
      if (this.currentScreen && this.currentScreen.setState) {
        this.currentScreen.setState({ 
          errorMessage: `Error showing Kubernetes context popup: ${error.message}` 
        });
      }
    }
  }
