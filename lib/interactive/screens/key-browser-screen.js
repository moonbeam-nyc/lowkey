...
class KeyBrowserScreen extends ComponentScreen {
        ...
        selectedIndex,
        {
          ...
          displayFunction: (key) => {
            ...
            if (showValues) {
              ...
              // Account for: selection indicator (2), key name, ": " separator (2), and some padding (4)
              const terminalWidth = process.stdout.columns || 80;
              const keyLength = key.length;
              const prefixLength = isSelected ? 2 : 0; // "✓ "
              const availableWidth = terminalWidth - keyLength - prefixLength - 2 - 4;
              
              // Truncate if needed, but use most of the available space
              const truncatedValue = displayValue.length > availableWidth && availableWidth > 3
                ? displayValue.substring(0, availableWidth - 3) + '...'
                : displayValue;
              
              // Format with key in default color and value in gray
              displayText = `${key}: ${colorize(truncatedValue, 'gray')}`;
            } else {
              displayText = key;
            }
  ...
  setupKeyHandlers() {
    ...
    const handlers = this.createKeyHandlers()
      ...
      // Base64 editing
      .onKey('\u0005', () => { // Ctrl+E - Edit base64 content
        if (!this.state.searchMode) {
  ...
  handleEditKeys() {
    ...
    if (inMultiSelectMode && selectedKeys.size > 0) {
      // Edit all selected keys
      keysToEdit = Array.from(selectedKeys);
    } else if (filteredKeys.length > 0) {
      // Edit only the currently focused key
      keysToEdit = [filteredKeys[selectedIndex]];
    } else {
      // No keys to edit
      return;
    }
    
    // Launch editor with the selected keys
    this.handleEdit(keysToEdit);
  }
      ...
      if (editorPromise) {
        ...
        if (result !== null) {
          ...
          if (result && typeof result === 'object' && 'changed' in result) {
            ...
            const keyCount = keysToEdit ? keysToEdit.length : Object.keys(this.secretData).length;
            const successMsg = keyCount === 1 
              ? 'Key updated successfully' 
              : `${keyCount} keys updated successfully`;
            this.setState({ 
              filteredKeys,
              successMessage: successMsg,
              selectedIndex: 0, // Reset selection
              selectedKeys: new Set(), // Clear selection after edit
              inMultiSelectMode: false // Exit multi-select mode
            });
