...
class FuzzySearchScreen extends Screen {
  ...
  renderItemList(output, items, selectedIndex, query, searchMode) {
    ...
    for (let i = startIndex; i < endIndex; i++) {
      ...
      // Only show selection indicator when not in search mode
      const prefix = (isSelected && !searchMode) ? colorize('> ', 'green') : '  ';
      const itemColor = (isSelected && !searchMode) ? 'bright' : 'reset';
