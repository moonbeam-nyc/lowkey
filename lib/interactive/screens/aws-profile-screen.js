...
const { getAvailableProfiles, getCurrentProfile, getCurrentRegion } = require('../../utils/aws-config');
...
class AwsProfilePopup extends BasePopup {
  ...
  constructor(options = {}) {
    ...
    try {
      ...
      this.currentRegion = getCurrentRegion() || 'unset';
  ...
  handleCompactMode(key) {
    ...
    const keyHandlers = new KeyHandlerSet()
      .onEscape(() => {
        ...
        // Apply the final configuration when closing
        this.applyConfiguration(this.currentProfile, this.currentRegion);
        this.close();
        return true;
      })
      .onKey('\u0003', () => { // Ctrl+C
    ...
    const keyHandlers = new KeyHandlerSet()
      ...
      .onEnter(() => {
        if (searchMode) {
          ...
          if (selectedProfile) {
            // Update current profile and return to compact mode
            this.currentProfile = selectedProfile;
            // Apply configuration but don't call onConfigChange yet since we're staying in the popup
            // Just update the environment variables
            if (selectedProfile) {
              process.env.AWS_PROFILE = selectedProfile;
            }
            this.setState({ mode: 'compact', query: '', searchMode: false, selectedFieldIndex: 0 });
          }
          return true;
        }
      })
    ...
    const keyHandlers = new KeyHandlerSet()
      ...
      .onEnter(() => {
        if (searchMode) {
          ...
          if (selectedRegion) {
            // Update current region and return to compact mode
            ...
            this.setState({ mode: 'compact', query: '', searchMode: false, selectedFieldIndex: 1 });
          }
          return true;
        }
      })
