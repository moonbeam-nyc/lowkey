...
class CopyWizardScreen extends ComponentScreen {
  constructor(options) {
    ...
    debugLogger.log('CopyWizardScreen.constructor', 'Copy wizard received filteredKeys', { 
      filteredKeys: this.filteredKeys, 
      filteredKeyCount: this.filteredKeys.length,
      totalSecretKeys: Object.keys(this.secretData).length 
    });
      ...
      .onKey('G', () => {
        if (!this.isInSearchMode() && !this.isInTextInput()) {
          ...
          if (currentItems.length > 0) {
            this.setState({ selectedIndex: currentItems.length - 1 });
          }
          return true;
        }
        return false; // Don't consume the key if we're in text input
      ...
      .onEscape(() => {
        ...
        if (step === 'done' || step === 'copying') {
          ...
          if (outputType === 'kubernetes') {
            this.setState({ step: 'namespace', selectedIndex: 0, outputNamespace: null, outputName: null });
          } else {
            this.setState({ step: 'type', selectedIndex: 0, outputType: null, outputName: null });
          }
        } else if (step === 'namespace') {
  ...
  getComponents(state) {
    ...
    if (step === 'type') {
      ...
      components.push(Text(`  ${outputType}`, 'gray'));
    }
    components.push(Spacer());
    ...
    if (outputType === 'kubernetes') {
      ...
      if (step === 'namespace') {
        ...
        components.push(Text(`  ${outputNamespace}`, 'gray'));
      }
      components.push(Spacer());
    }
    ...
    if (outputType === 'kubernetes') {
      if (outputNamespace || step === 'secret') {
        ...
        if (step === 'secret') {
          ...
          components.push(Text(`  ${outputName}`, 'gray'));
        }
        components.push(Spacer());
      }
  ...
  selectOutputType() {
    ...
    if (selectedType === 'kubernetes') {
      ...
      this.setState({ 
        outputType: selectedType,
        step: 'file',
        selectedIndex: 0  // Ensure cursor starts at first item
      });
    }
  }
  ...
  async performCopy() {
    try {
      ...
      const copyOptions = {
        ...
        secretData: this.secretData, // Provide the pre-fetched secret data
        filteredKeys: this.filteredKeys // Pass the selected keys
      };
  ...
  getConfirmComponents(state) {
    ...
    const components = [
      ...
      Spacer(),
      // Add help text explaining merge behavior
      Text('New keys will be added to existing files', 'gray'),
      Text('Existing keys with same names will be overwritten', 'gray'),
      Spacer(),
  ...
  getFileOptions() {
    ...
    try {
      ...
      // Filter files based on output type using the proper file listing functions
      let filteredFiles = [];
      if (outputType === 'env') {
        const { listEnvFiles } = require('../../providers/files');
        filteredFiles = listEnvFiles(currentDir);
      } else if (outputType === 'json') {
        const { listJsonFiles } = require('../../providers/files');
        filteredFiles = listJsonFiles(currentDir);
      }
