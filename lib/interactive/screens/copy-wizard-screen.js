    ...
    const handler = async (keyStr, state) => {
      ...
      if (keyStr === '\u001b') { // Escape - go back
        if (step === 'preview') {
          ...
          // For errors, go back to previous step to fix the issue
          this.previousStep();
        } else {
          // Go to previous step
          this.previousStep();
      ...
      switch (step) {
        ...
        case 'error':
          if (keyStr === '\r') { // Enter - go back to previous step to fix error
            this.previousStep();
          }
          // Note: ESC is handled by the common ESC handler above which calls previousStep()
          break;
      }
