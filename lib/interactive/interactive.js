          ...
          if (code === 0) {
            ...
            try {
              ...
              if (originalContent === newContent) {
                ...
                resolve({ changed: false, data: null });
              } else {
                try { fs.unlinkSync(tempFile); } catch (cleanupError) { }
                resolve({ changed: true, data: editedData });
              }
            } catch (parseError) {
              try { fs.unlinkSync(tempFile); } catch (cleanupError) { }
      ...
      editorProcess.on('exit', (code) => {
        try {
          if (code === 0) {
            ...
            // Check if content actually changed by comparing original and edited data
            const originalContent = generateEnvContent(dataToEdit);
            const newContent = generateEnvContent(editedData);
          ...
          if (code === 0) {
            ...
            try {
              ...
              if (secretName && namespace) {
                try {
                  const { setSecret } = require('../providers/kubernetes');
                  const finalData = { ...secretData, ...editedData };
...
async function editBase64Content(decodedContent, keyName) {
  return new Promise((resolve, reject) => {
    const tempFile = path.join(os.tmpdir(), `lowkey-base64-edit-${keyName}-${Date.now()}.txt`);

    try {
      fs.writeFileSync(tempFile, decodedContent);
      const editor = config.getEditor();
