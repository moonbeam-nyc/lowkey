...
class PopupManager {
  ...
  showPopup(popup, baseScreen) {
    ...
    try {
      ...
      // Capture current screen content BEFORE showing popup to avoid re-render pagination issues
      this.captureBaseContent();
  ...
  closePopup() {
    ...
    this.baseScreen = null;
    this.originalKeyHandlers = [];
    this.lastRenderedContent = null; // Clear cached content

    // Restore cursor visibility before re-rendering
  ...
  /**
   * Capture current base screen content to avoid re-render issues
   */
  captureBaseContent() {
    const debugLogger = require('../core/debug-logger');
    try {
      if (this.baseScreen && this.baseScreen.getComponents) {
        // Get the screen's current rendered output by calling its render method
        // This avoids duplication issues with header/breadcrumb handling
        if (typeof this.baseScreen.render === 'function') {
          ...
          try {
            // Trigger the screen's own render method which handles everything correctly
            this.baseScreen.render();
            this.lastRenderedContent = capturedOutput;
            debugLogger.log('Captured base content via screen render');
          } finally {
            // Always restore stdout
            process.stdout.write = originalWrite;
          }
        } else {
          debugLogger.log('Screen has no render method, falling back to null');
          this.lastRenderedContent = null;
        }
      }
    } catch (error) {
      debugLogger.log('Error capturing base content', { error: error.message });
      this.lastRenderedContent = null; // Reset on error
    }
  }
  ...
  render() {
    ...
    try {
      ...
      // Get base screen content WITHOUT re-rendering to avoid pagination changes
      let baseContent = '';
      
      // Capture current screen output instead of re-rendering
      const { terminal } = require('./terminal-utils');
      
      // Try to get the last rendered content from the terminal buffer
      // This avoids the pagination issue caused by creating new renderer instances
      if (this.lastRenderedContent) {
        baseContent = this.lastRenderedContent;
        debugLogger.log('Using cached base content to avoid re-render');
      } else {
        // Fallback: get current screen content, but this is not ideal
        if (this.baseScreen.getComponents) {
          // Component-based screen - use TerminalManager's renderComponents which includes header
          const { TerminalManager } = require('./terminal-manager');
          const terminalManager = TerminalManager.getInstance();
          
          // Get components from screen
          const components = this.baseScreen.getComponents(this.baseScreen.state);
          
          // Render using terminal manager which includes header
          const { ComponentRenderer } = require('./component-renderer');
          const componentRenderer = new ComponentRenderer();
          
          // Extract breadcrumbs from components
          let breadcrumbs = [];
          ...
          // Get header lines from terminal manager
          const headerLines = terminalManager.getHeaderLines(breadcrumbs);
          
          // Render the header and components together
          const headerContent = headerLines.join('\n');
          const bodyContent = componentRenderer.render(components) || '';
          baseContent = headerContent + '\n' + bodyContent;
          
          debugLogger.log('Used component renderer with header for base content (fallback)');
        } else if (this.baseScreen.renderer && this.baseScreen.renderer.renderFunction) {
          // Legacy screen with renderFunction
          baseContent = this.baseScreen.renderer.renderFunction(this.baseScreen.state) || '';
          debugLogger.log('Used legacy render function for base content');
        } else {
      ...
      // Hide cursor to avoid it appearing at the end of popup box
  ...
  overlayContent(baseLines, popupLines, position, popupDimensions) {
    ...
    // Use base lines exactly as they are - don't extend to terminal height
    // This prevents adding extra lines that push content around
    ...
    // Process only the lines that actually exist in base content or are needed for popup
    const maxLines = Math.max(workingBaseLines.length, position.y + popupLines.length);
    ...
    for (let i = 0; i < maxLines; i++) {
      ...
      if (popupLineIndex >= 0 && popupLineIndex < popupLines.length) {
        ...
        // Use base content as-is, but only if it exists
        if (i < workingBaseLines.length) {
