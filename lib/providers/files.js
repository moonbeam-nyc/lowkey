  ...
  try {
    ...
    for (const line of lines) {
      ...
      if (trimmed && !trimmed.startsWith('#')) {
        const match = trimmed.match(config.get('env.linePattern'));
        if (match) {
          let [, key, value] = match;
...
function listEnvFiles(directory) {
  try {
    ...
    const envFiles = files.filter(file => {
      ...
      // Match files that start with .env OR end with .env
      return fs.statSync(fullPath).isFile() && (file.match(/^\.env/) || file.match(/\.env$/));
    });
...
function listJsonFiles(directory) {
  try {
    ...
    const excludeFiles = config.get('files.jsonExcludes');
...
function validateEnvKey(key) {
  return config.get('env.keyPattern').test(key);
}
...
function backupFile(filePath) {
  if (fs.existsSync(filePath)) {
    const backupPath = `${filePath}${config.get('files.backupExtension')}`;
