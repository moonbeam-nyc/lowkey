...
const { KUBERNETES } = require('../core/constants');
...
async function getAvailableContexts() {
  try {
    const output = await executeKubectl(['config', 'get-contexts', '--output=name']);
...
async function switchContext(contextName) {
  try {
    await executeKubectl(['config', 'use-context', contextName]);
    return contextName;
  } catch (error) {
