  ...
  async store(name, data, options = {}) {
    ...
    try {
      ...
      if (fs.existsSync(name)) {
        ...
        try {
          const existingData = await this.fetch(name, options);
          // Merge: new keys overwrite existing ones, existing keys are preserved if not in new data
          finalData = { ...existingData, ...data };
        } catch (error) {
          // If we can't read the existing file, just use the new data
          console.error(colorize(`Warning: Could not read existing file for merge: ${error.message}`, 'yellow'));
        }
      }
      ...
      const totalKeys = Object.keys(finalData).length;
      return `Successfully ${fs.existsSync(name + '.bak') ? 'merged' : 'wrote'} ${keyCount} key${keyCount === 1 ? '' : 's'} to ${name} (${totalKeys} total keys)`;
    } catch (error) {
...
class EnvFileProvider extends SecretProvider {
  ...
  async store(name, data, options = {}) {
    ...
    try {
      ...
      // Generate env content from merged data
      const lines = [];
